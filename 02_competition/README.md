...
過去のデータ（Xヵ月：20XX-XX-XX 00:00～20XX-XX-XX 23:00）を用いて1時間単位の在室人数を予測するモデルを構築し、未来のデータ（Xヵ月：20XX-XX-XX 00:00～20XX-XX-XX 23:00）における在室人数を予測した結果を提出してください。
...
- **提出物:** 対象期間内の予測結果を記述したCSVファイル（`submission.csv`）
    ...
    YYYY-MM-DD 01:00:00,0
    YYYY-MM-DD 02:00:00,0
    ...
    ```

`submission`ディレクトリの`_generate_submission_file.py`を使用すると、テスト期間に応じた `submission.csv` のテンプレートを生成できます。ファイル内の `START_DATETIME` と `END_DATETIME` を適切な期間に設定してから実行してください。
...
なお、評価対象となる期間は`submission.csv`の期間に関わらず、**X週間：YYYY-MM-DD HH:MM～YYYY-MM-DD HH:MM のみ**のため、全てのテスト期間に対する評価結果ではないことに注意してください。

⚠️注意：ファイル内の `API_URL` と `USER_ID` を環境に合わせて設定してから実行してください。

- ベースライン（全て0と予測した場合）
```text
~/workspace/02_competition/submission$ python3 _evaluate_submission.py
📡 サーバー(http://172.16.15.XXX:5000/evaluate)にデータを送信中...
========================================
✅ 評価結果 (Success)
----------------------------------------
  [Main Score]
  Weighted MAE     : X.XXXXXX
----------------------------------------
  [Time Segmented MAE]
  Day (09-18)      : X.XXXXXX
  Night (18-24)    : X.XXXXXX
  Midnight (00-09) : X.XXXXXX
----------------------------------------
  [Global Details]
  RMSLE            : X.XXXXXX
  MAE (Global)     : X.XXXXXX
...
⚠️注意：このコードは**ユーザーごとに1日に実行できる回数（上限X回/日）が決まっている**ので、慎重に実行してください。